...
local function OpenPlayerMenus(player)
    ...
    local elements = {
        ...
        [13] = {
            icon = '⛓️',
            label = 'Encarcelar (Jail)',
            value = 'jail',
            description = 'Enviar a prisión de Bolingbroke con tiempo'
        },
        [14] = {
            icon = '🔓',
        ...
        Players:AddButton({
            ...
            select = function(btn)
                ...
                elseif values == 'jail' then
                    local time = LocalInputInt('Minutos en Prisión de Bolingbroke', 4)
                    local reason = LocalInput('Motivo del Encarcelamiento', 50, 'Sanción Staff')
                    if time and time > 0 then
                        ExecuteCommand('jail ' .. player.id .. ' ' .. time .. ' ' .. (reason or 'Sanción Staff'))
                    end
